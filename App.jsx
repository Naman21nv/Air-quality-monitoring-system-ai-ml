...
const AirQuality = () => {
    ...
    const fetchAQData = async (cityName) => {
        ...
        try {
            ...
            const response = await axios.post(`${BACKEND_URL}/get_aqi_data`, {
                city: cityName
            });

            if (response.data) {
                const waqiData = response.data.waqi_data;
                const predictionData = response.data.prediction;

                setAqData(waqiData);
                setCity(cityName);

                // Update model prediction state
                setModelPrediction({
                    source: predictionData.source,
                    health_impact: predictionData.health_impact,
                    precautionary_measures: predictionData.precautionary_measures,
                    aqi: parseFloat(predictionData.aqi) || 0
                });

                // Update AQI and related states
                const aqi = parseFloat(predictionData.aqi) || 0;
                setMlResult({ aqi });
                setHealthPrecautions(getHealthPrecautions(aqi));
                setPollutionSources(getPollutionSources(cityName));

            } else {
                setError("City not found or API limit exceeded.");
            ...
            if (err.code === 'ERR_NETWORK' || err.code === 'ERR_FAILED') {
                // First, let's check if our backend is accessible
                try {
                    ...
                    if (backendCheck.status === 200) {
                        errorMessage = "Cannot connect to air quality service. Please check:\n" +
                            "1. Your internet connection\n" +
                            "2. VPN or proxy settings (if any)\n" +
                            "3. Try refreshing the page";
                    } else {
                        errorMessage = `Backend server responded with status ${backendCheck.status}. Please ensure:\n` +
                            "1. Backend server is running on port 5000\n" +
                            "2. No other application is using port 5000\n" +
                            "3. Your firewall allows connections to localhost:5000";
                    }
                } catch (backendErr) {
                ...
                if (err.response.status === 401) {
                    ...
                    errorMessage = `No air quality data available for "${cityName}". Please try another city or check the spelling.`;
                } else if (err.response.status === 429) {
                    errorMessage = "API rate limit exceeded. Please wait a few minutes before trying again.";
                } else {
                    errorMessage = err.response.data?.message || `Server error (${err.response.status}): ${err.response.data?.error || 'Unknown error'}`;
                }
                ...
                errorMessage = "Request was sent but no response received. This might indicate:\n1. Server is down\n2. Request timed out\n3. CORS policy blocking the response";
                console.error("No response error:", {
                    request: err.request,
                    config: err.config
                });
            } else {
                errorMessage = err.message || "An unexpected error occurred while processing your request.";
            ...
            {aqData && mlResult && (
                <>
                        ...
                        <div className="card">
                            ...
                            <div className="aqi-card-content">
                                ...
                                <div className="prediction-details">
                                    <p><strong>Source:</strong> {mlResult.source}</p>
                                    <p><strong>Health Impact:</strong> {mlResult.healthImpact}</p>
                                    <p><strong>Precautions:</strong> {mlResult.precautionaryMeasures}</p>
                                </div>
                    ...
                    <div className="dashboard-grid">
                        ...
                        <div className="model-predictions-card card">
                            <h3 className="card-title">AI Model Predictions</h3>
                            <div className="prediction-content">
                                <div className="prediction-item">
                                    <span className="prediction-label">Main Pollution Source:</span>
                                    <span className="prediction-value">{modelPrediction.source}</span>
                                </div>
                                <div className="prediction-item">
                                    <span className="prediction-label">Health Impact:</span>
                                    <span className="prediction-value">{modelPrediction.health_impact}</span>
                                </div>
                                <div className="prediction-item">
                                    <span className="prediction-label">Precautionary Measures:</span>
                                    <span className="prediction-value">{modelPrediction.precautionary_measures}</span>
                                </div>
                                <div className="prediction-item">
                                    <span className="prediction-label">Calculated AQI:</span>
                                    <span className="prediction-value" style={{ color: getAqiStatus(modelPrediction.aqi).color }}>
                                        {modelPrediction.aqi}
                                    </span>
                                </div>
                            </div>
